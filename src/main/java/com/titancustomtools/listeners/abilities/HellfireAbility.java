...
public class HellfireAbility {
    ...
    public void handleCast(PlayerFishEvent event) {
        ...
        task.runTaskTimer(plugin, 1, 5);
        activeTasks.put(player.getUniqueId(), task);
    }

    // UPDATED: Now accepts the ItemStack rod
    public void handleReel(PlayerFishEvent event, ItemStack rod) {
        Player player = event.getPlayer();
        ...
        if (task.state == FishingState.BITING) {
            ...
            // DECREMENT USE
            plugin.getToolManager().decrementUse(player, rod);
        }
    }
    ...
    private class LavaFishingTask extends BukkitRunnable {
        ...
        public void run() {
            ...
            if (state == FishingState.WAITING) {
                waitTimer--;
                player.sendTitle(ChatColor.GOLD + "Catching...", ChatColor.YELLOW + "Wait for it...", 0, 20, 10);

                if (waitTimer <= 0) {
                    state = FishingState.BITING;
                    biteWindowTimer = 6 + random.nextInt(10);
                    player.sendTitle(ChatColor.GREEN + "!!! BITE !!!", ChatColor.AQUA + "RIGHT CLICK NOW!", 0, 20, 10);
